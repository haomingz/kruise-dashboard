...
export function WorkloadTabs() {
  ...
  const getColumns = (showImage: boolean = true) => [
    {
      ...
      render: (text: string, record: WorkloadItem) => (
        <Typography.Link onClick={() => handleWorkloadClick(record)}>{text}</Typography.Link>
      ),
    },
    ...
    { title: 'Age', dataIndex: 'age', key: 'age' },
    {
      ...
      render: (_: any, record: WorkloadItem) => (
        <Button
          ...
          size="small"
          icon={<LinkOutlined />}
          onClick={() => handleWorkloadClick(record)}
        >
          View
        </Button>
      ),
  ...
  // Handle navigation to workload detail page
  const handleWorkloadClick = (record: WorkloadItem) => {
    const workloadId = `${record.workloadType}-${record.namespace}-${record.name}`;
    router.push(`/workloads/${workloadId}`);
  };
